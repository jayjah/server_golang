...
var HolidayTableColumns = struct {
	ID          string
	Name        string
	Description string
	Date        string
...
var HolidayWhere = struct {
	...
	ID:          whereHelperint64{field: "\"holidays\".\"id\""},
	Name:        whereHelperstring{field: "\"holidays\".\"name\""},
	Description: whereHelperstring{field: "\"holidays\".\"description\""},
	Date:        whereHelpertime_Time{field: "\"holidays\".\"date\""},
}
...
var (
	holidayAllColumns            = []string{"id", "name", "description", "date"}
	holidayColumnsWithoutDefault = []string{"name", "description", "date"}
	holidayColumnsWithDefault    = []string{"id"}
	holidayPrimaryKeyColumns     = []string{"id"}
	holidayGeneratedColumns      = []string{}
)

type (
	// HolidaySlice is an alias for a slice of pointers to Holiday.
	// This should almost always be used instead of []Holiday.
...
func (q holidayQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Holiday, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for holidays")
	}
...
func (q holidayQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count holidays rows")
	}
...
func (q holidayQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if holidays exists")
	}
...
func Holidays(mods ...qm.QueryMod) holidayQuery {
	mods = append(mods, qm.From("\"holidays\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"holidays\".*"})
	}

	return holidayQuery{q}
}

// FindHoliday retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindHoliday(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Holiday, error) {
	holidayObj := &Holiday{}
	...
	query := fmt.Sprintf(
		"select %s from \"holidays\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from holidays")
	}

	if err = holidayObj.doAfterSelectHooks(ctx, exec); err != nil {
		return holidayObj, err
	}
...
func (o *Holiday) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no holidays provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"holidays\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"holidays\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into holidays")
	}
...
func (o *Holiday) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update holidays, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"holidays\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update holidays row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for holidays")
	}
...
func (q holidayQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for holidays")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for holidays")
	}
...
func (o HolidaySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"holidays\" SET %s WHERE %s",
...
func (o *Holiday) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no holidays provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert holidays, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"holidays\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert holidays")
	}
...
func (o *Holiday) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from holidays")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for holidays")
	}
...
func (q holidayQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from holidays")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for holidays")
	}
...
func (o HolidaySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"holidays\" WHERE " +
...
func (o *HolidaySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"holidays\".* FROM \"holidays\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, holidayPrimaryKeyColumns, len(*o))
...
func HolidayExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"holidays\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if holidays exists")
	}

	return exists, nil
}

// Exists checks if the Holiday row exists.
func (o *Holiday) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return HolidayExists(ctx, exec, o.ID)
}
